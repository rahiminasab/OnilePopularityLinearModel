...
<<echo=false>>=
pr_ts <- predict(m1_2,ts[,-ncol(ts)])
mspr_ts <- rmse(pr_ts,ts[,ncol(ts)])^2
@
<<>>=
mspr_ts
@
...
dt <- data[,c(2,3,5,10,11,13:19,39,45:46, ncol(data)-1)]
colnames(dt) <- c(paste("X",1:(ncol(dt)-1), sep=""),"y")
@
<<plot_plotboxcox,fig=TRUE>>=
boxcox(y~.,data=dt)
@
